<!-- admin settings UI. seeds from the current settings and saves via onUpdate. -->
<script lang="ts">
  import { untrack } from 'svelte'
  import type { SettingsComponentProps } from '$lib/plugins/_core/types.client'
  import { defaultHelloSettings } from '../schema'

  let { settings, onUpdate, loading = false, error }: SettingsComponentProps = $props()

  let greeting = $state(
    untrack(() => (settings.greeting as string) ?? defaultHelloSettings.greeting),
  )
</script>
